...
class TestPriorityFromLabels:
    ...
    def test_empty_labels(self) -> None:
        """Test empty labels returns medium."""
        assert _priority_from_labels([]) == "medium"

    def test_high_priority_labels(self) -> None:
        """Test high priority labels."""
        assert _priority_from_labels(["priority:high"]) == "high"
        assert _priority_from_labels(["critical"]) == "high"
        assert _priority_from_labels(["urgent"]) == "high"
        assert _priority_from_labels(["p0"]) == "high"
        assert _priority_from_labels(["p1"]) == "high"
        assert _priority_from_labels(["high-priority"]) == "high"

    def test_low_priority_labels(self) -> None:
        """Test low priority labels."""
        assert _priority_from_labels(["priority:low"]) == "low"
        assert _priority_from_labels(["minor"]) == "low"
        assert _priority_from_labels(["p3"]) == "low"
        assert _priority_from_labels(["p4"]) == "low"
        assert _priority_from_labels(["low-priority"]) == "low"
        assert _priority_from_labels(["nice-to-have"]) == "low"

    def test_unknown_labels(self) -> None:
        """Test unknown labels return medium."""
        assert _priority_from_labels(["bug"]) == "medium"
        assert _priority_from_labels(["enhancement"]) == "medium"
    ...
    def test_multiple_labels(self) -> None:
        ...
        assert _priority_from_labels(["bug", "critical", "p3"]) == "high"
...
class TestLoadGithubTasks:
    ...
    def test_skips_issues_without_title(self) -> None:
        ...
        with patch("subprocess.run") as mock_run:
            ...
            tasks = load_github_tasks()
            assert len(tasks) == 1
            assert tasks[0].description == "Has title"

    def test_metadata_preserved(self) -> None:
        """Test that original issue data is preserved as metadata."""
        issues = [
            {
                "number": 1,
                "title": "Issue",
